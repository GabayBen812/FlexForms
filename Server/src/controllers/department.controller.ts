...
export const deleteDepartment = async (
  ...
  try {
    ...
    if (!result) res.status(404).json({ message: "Department not deleted" });
    else res.status(200).json({ message: "Department deleted" });
  } catch (error) {
...
export const updateDepartment = async (req: ExtendedRequest, res: Response) => {
  ...
  try {
    ...
    if (!result) res.status(404).json({ message: "Department not updated" });
    else res.status(200).json({ message: "Department updated" });
  } catch (error) {
